...
fermentacion_bp=Blueprint("fermentacion",__name__)
@fermentacion_bp.route("/fermentacion",methods=["POST"])
def cargar():
...
@fermentacion_bp.route("/obtener_datos_fermentacion", methods=["GET"])
def obtener_datos():
...
@fermentacion_bp.route("/fermentacion/<int:id>",methods=["POST","GET"])
def modificar(id):
...
@fermentacion_bp.route("/fermentacion/<int:id>", methods=["DELETE"])
def borrar(id):
    datos = Fermentacion.query.get_or_404(id)
